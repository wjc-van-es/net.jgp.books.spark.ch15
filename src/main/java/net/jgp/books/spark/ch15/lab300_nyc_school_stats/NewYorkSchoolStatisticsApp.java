...
public class NewYorkSchoolStatisticsApp {
    ...
    private void start() {
        ...
        Dataset<Row> averageEnrollmentDf = masterDf
                ...
                .withColumn("Average Enrolled", round(col("avg(enrolled)"), 0))
                .withColumn("Average Present", round(col("avg(present)"), 0))
                .withColumn("Average Absent", round(col("avg(absent)"), 0))
                //.drop("avg(enrolled)", "avg(present)", "avg(absent)")
                ;
        ...
        absenteeRatioDf = absenteeRatioDf
                ...
                .withColumn("%", round(expr("avg_absent / avg_enrolled * 100"), 4))
                .withColumn("avg_enrolled", round(col("avg_enrolled"),0)) //round after calculating the %
                .withColumn("avg_absent", round(col("avg_absent"),4)) //round after calculating the %
                //.filter(col("avg_enrolled").$greater$eq(9)) //ensures records under 9 are filtered out
                .filter(col("avg_enrolled").geq(9)) //the same as the previous filter ensures records under 9 are filtered out
                .orderBy("%", "avg_enrolled");
        log.info("Schools with the least absenteeism");
        absenteeRatioDf.show(15);
